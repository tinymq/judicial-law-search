/**
 * 阶段2A: Import 江苏地方性法规 from flk.npc.gov.cn
 *
 * Usage: node scripts/import-jiangsu-local-regs.js [--dry-run] [--limit N] [--skip-fetch]
 *
 * Steps:
 * 1. Fetch all valid 江苏地方性法规 from flk API (paginated)
...
const CACHE_PATH = path.join(__dirname, 'data', 'flk-jiangsu-local-regs.json');
const MISSING_PATH = path.join(__dirname, 'data', 'flk-jiangsu-missing.json');
...
// ============ Region detection ============
function detectRegion(zdjgName, title) {
  const cityMatch = (zdjgName || '').match(/^(.{2,3})[市州]人民代表大会/);
  if (cityMatch) return cityMatch[1];
  const titleMatch = (title || '').match(/^(.{2,3})[市州]/);
...
async function fetchAllFromFlk() {
  if (fs.existsSync(CACHE_PATH)) {
    console.log('Using cached flk data from', CACHE_PATH);
    return JSON.parse(fs.readFileSync(CACHE_PATH, 'utf-8'));
  }

  console.log('Fetching 江苏地方性法规 from flk.npc.gov.cn...');
  const allRows = [];
  const PAGE_SIZE = 20;
  let page = 1;
  let total = 0;
  ...
  fs.writeFileSync(CACHE_PATH, JSON.stringify(allRows, null, 2), 'utf-8');
  console.log(`Saved ${allRows.length} entries to ${CACHE_PATH}\n`);
  return allRows;
}

// ============ Phase 2: Compare with DB ============
function findMissing(flkRows, db) {
  // Check against ALL laws (not just region='江苏') to avoid any duplicates
  const allLaws = db.prepare(`SELECT id, title FROM Law`).all();
  const localNormSet = new Set(allLaws.map(l => normalize(l.title)));

  const jsLocalCount = db.prepare(`SELECT COUNT(*) as c FROM Law WHERE level = '地方性法规' AND region = '江苏'`).get().c;
  console.log(`本地 江苏省级地方性法规: ${jsLocalCount}`);
...
async function processItem(db, item, cookie, index, total, retries = 0) {
  ...
  try {
    ...
    const region = detectRegion(item.zdjgName, item.title);
    const lawData = {
      title: titleWithYear,
...
async function main() {
  ...
  console.log(`=== 阶段2A: 江苏地方性法规补全 ===`);
  console.log(`Dry run: ${dryRun}, Limit: ${limit === Infinity ? 'none' : limit}\n`);

  // Phase 1: Fetch
  let flkRows;
  if (skipFetch && fs.existsSync(CACHE_PATH)) {
    flkRows = JSON.parse(fs.readFileSync(CACHE_PATH, 'utf-8'));
    console.log(`Loaded ${flkRows.length} cached entries\n`);
  } else {
    flkRows = await fetchAllFromFlk();
  }
  ...
  const finalCount = db.prepare(`SELECT COUNT(*) as c FROM Law WHERE level = '地方性法规' AND region = '江苏'`).get().c;
  console.log(`江苏地方性法规 total after import: ${finalCount}`);
