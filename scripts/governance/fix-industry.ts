 ...
 *
 * 根据法规标题和制定机关自动匹配行业分类
 * 写入 LawIndustry 多对多关联表
 *
 * 用法：
 *   npx tsx scripts/governance/fix-industry.ts          # 分析模式
 *   npx tsx scripts/governance/fix-industry.ts --apply   # 执行修复
 */

import { PrismaClient } from '@prisma/client';
import path from 'path';

// 内联 matchIndustries（避免 ESM/CJS 问题）
type IndustryKW = { code: string; name: string; authorities: string[]; titleKeywords: string[] };

const INDUSTRY_KEYWORDS: IndustryKW[] = [
...
function matchIndustries(title: string, authority?: string | null): Array<{ code: string; name: string; isPrimary: boolean }> {
...
async function main() {
  ...
  console.log(`行业数: ${industries.length}`);

  // 获取所有法规
  const laws = await prisma.law.findMany({
    select: { id: true, title: true, issuingAuthority: true, industryId: true },
  });
  console.log(`法规数: ${laws.length}`);

  // 检查已有的 LawIndustry 记录
  const existingCount = await prisma.lawIndustry.count();
  console.log(`已有 LawIndustry 记录: ${existingCount}\n`);

  // 分析分类
  const distribution = new Map<string, number>();
  const toInsert: Array<{ lawId: number; industryId: number; isPrimary: boolean }> = [];
  let otherCount = 0;
  let multiCount = 0;

  for (const law of laws) {
    const matched = matchIndustries(law.title, law.issuingAuthority);

    for (const m of matched) {
      const ind = industryByCode.get(m.code);
      if (!ind) continue;
      toInsert.push({ lawId: law.id, industryId: ind.id, isPrimary: m.isPrimary });
      distribution.set(m.name, (distribution.get(m.name) || 0) + 1);
    }

    if (matched.length > 1) multiCount++;
    if (matched[0]?.code === "99") otherCount++;
  }

  console.log(`=== 分析结果 ===`);
  console.log(`将插入 LawIndustry 记录: ${toInsert.length}`);
  console.log(`多行业标记的法规: ${multiCount}`);
  console.log(`归入"其他"的法规: ${otherCount}`);
  ...
  // 执行：清空旧记录，重新插入
  console.log(`\n🔧 清空旧 LawIndustry 记录...`);
  await prisma.lawIndustry.deleteMany();

  console.log(`🔧 插入 ${toInsert.length} 条 LawIndustry 记录...`);
  const BATCH_SIZE = 100;
  let processed = 0;

  for (let i = 0; i < toInsert.length; i += BATCH_SIZE) {
    const batch = toInsert.slice(i, i + BATCH_SIZE);
    await prisma.lawIndustry.createMany({
      data: batch,
    });
    processed += batch.length;
    if (processed % 5000 === 0 || processed === toInsert.length) {
      console.log(`  进度: ${processed}/${toInsert.length}`);
    }
  }

  // 更新 Law.industryId（主分类）
  console.log(`\n🔧 更新 Law.industryId（主分类）...`);
  const primaryRecords = toInsert.filter(r => r.isPrimary);
  let updatedCount = 0;
  for (let i = 0; i < primaryRecords.length; i += BATCH_SIZE) {
    const batch = primaryRecords.slice(i, i + BATCH_SIZE);
    await prisma.$transaction(
      batch.map(r => prisma.law.update({
        where: { id: r.lawId },
        data: { industryId: r.industryId },
      }))
    );
    updatedCount += batch.length;
    if (updatedCount % 5000 === 0 || updatedCount === primaryRecords.length) {
      console.log(`  进度: ${updatedCount}/${primaryRecords.length}`);
    }
  }

  const finalCount = await prisma.lawIndustry.count();
  const noIndustry = await prisma.law.count({ where: { industryId: null } });
  console.log(`\n✅ 修复完成！`);
  console.log(`LawIndustry 记录总数: ${finalCount}`);
  console.log(`剩余无 industryId: ${noIndustry}`);
}
