...
**当前数据量（v2.1.2，2026-04-24）：**
- 法规总数：**7,766 部**（治理后；治理前 7,827）
- lawGroupId 覆盖：**100%**（NULL = 0）
- 标准年份标记覆盖：**99.8%**（7,753 / 7,766，13 部因无任何日期跳过）
- baseTitle 多成员 cluster：39 个（正常的法规版本族）
- 行业分类：71 个一级行业
- 法规-行业关联：4267 条
- 效力位阶：11 个 · 时效性：4 个
- 执法事项：1000 条（湖南）· 全部已关联法规
- 数据库大小：~340 MB
...
- **v2.1.2** (2026-04-24): 法规数据治理 — 清理 61 对近重复（274 条 EnforcementItem 重定向 + 61 条 Law 删除）、补齐 1,145 部 lawGroupId（NULL → 0）、规范化 + 补齐 7,329 部标题年份标记。总数 7,827 → 7,766
- **v2.1.0** (2026-04-09): 执法事项模块升级（详情页、多法规动态关联、手动关联编辑、关联 API），1000 条执法事项全部关联法规
